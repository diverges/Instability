...
public class GateEntity extends BlockEntity {
	...
	public GateEntity(GameCanvas game, String folder, String ref,
			String onDeath, int x, int y, int width, int height, Item killWith,
			int quantity, String onDeathObj, int health) {
		super(game, folder, ref, onDeath, x, y, width, height, null, health,
			false);
	...
	public int dealDamage(int damage) {
		if (game.player.getInventory().hasEnough(this.killWith,
			this.quantityNeeded) != null) { return super.dealDamage(damage); }
		return 0;
	}
