...
public class BlockEntity extends WallEntity implements Killable {
	...
	public BlockEntity(GameCanvas game, String folder, String ref,
			String onDeath, int x, int y, int width, int height,
			Item onDeathDrop, int health, boolean respawn) {
	...
	public void collision() {
		...
		if (isAlive) {
			if (player.isMoving()) {
				if ((isBeingCollidedDown && player.isMovingDown())
						|| (isBeingCollidedUp && player.isMovingUp())
						|| (isBeingCollidedLeft && player.isMovingLeft())
						|| (isBeingCollidedRight && player.isMovingRight())) {
					Point playerPoint = player.getPoint();
					double distance = new Point(playerPoint.x,
						(int) (playerPoint.y - player.getHeight() / 2))
						.distance(this.getPoint());
	...
	public void act() {
		super.act();
		if (isAlive) {
			if (this.health <= 0) onDeath(); // trigger on death
		} else if (respawn) {
	...
	public void deathDrop() {
		if (CosmosConstants.debug) System.out.println("DEATH!");
		if (this.onDeathDrop != null) {
			game.level.getLevelInteractiveObjects().add(
				new DroppedItemEntity(game, null, onDeathDrop.getRef(), null,
					(int) me.x, (int) me.y, (int) me.width, (int) me.height,
					-1, onDeathDrop, 1, false));
	...
	public int dealDamage(int damage) {
		if (sprites.length > 1) {
			...
			if (step != 0) setCurrent(step);
		}
		this.health -= damage;
		if (CosmosConstants.debug) System.out.println(health);
		return damage;
	}
