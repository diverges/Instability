...
public class ItemSlot {
	...
	public boolean isFull() {
		if (item != null) return !item.isStackable() && quantity == max;
		return quantity == max;
	}
	...
	public String combine(ItemSlot cS2, ItemSlot pS) {
		...
		for (Combination c : CosmosConstants.COMBINATIONS) {
			if (c.getBlockName1().equals(this.item.getName())
					&& c.getBlockName2().equals(cS2Item.getName())) {
				if (c.getQuantityNeeded1() == this.getQuantity()
						&& c.getQuantityNeeded2() == cS2.getQuantity()) {
					pS.add(c.getItemMade(), c.getQuantityMade());
					combined = true;
					break;
				}
			} else if (c.getBlockName1().equals(cS2Item.getName())
					&& c.getBlockName2().equals(this.item.getName())) {
				if (c.getQuantityNeeded1() == cS2.getQuantity()
						&& c.getQuantityNeeded2() == this.getQuantity()) {
					pS.add(c.getItemMade(), c.getQuantityMade());
					combined = true;
					break;
				}
