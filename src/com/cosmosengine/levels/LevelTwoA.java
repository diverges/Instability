...
public class LevelTwoA extends LevelLoader {
	...
	public LevelTwoA(GameCanvas game) {
		...
		levelStartScript = new MessagePopupEntity(game, 50,
			"Scanners detect a large-n" + "amount of radiation being-n"
					+ "emitted by an unknown-n" + "source in the cavern.", null);

		enemyScript = new MessagePopupEntity(game, 50,
			"Leftover defence droid-n" + "from the mining operation.-n"
					+ "Their blades can harm-n" + "our armor.", null);

		uraniumScript = enemyScript = new MessagePopupEntity(game, 70,
			"Source of radiation confirmed.-n-n"
					+ "Uranium isotope located ahead.-n"
					+ "Although not the expected-n"
					+ "Radonite, substance may -n"
					+ "prove valuable in our search.", null);
