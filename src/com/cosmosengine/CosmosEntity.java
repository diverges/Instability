...
public abstract class CosmosEntity {
	 ...
	 * 
	 * @param game
	 *            used to access main class.
	 * @param folder
	 *            which contains the images
	 * @param ref
	 *            the name of the images separated by a ';' i.e. name1.png;name2.png;name3.png
	 * @param onDeath
	 *            the name of the images to animate when dead. Must be in folder/death/ folder. Same format as ref.
	 * @param x
	 *            x-pos of the object
	 * @param y
	 *            y-pos of the object
	 ...
	 *            of the object
	 * @param millis
	 *            on to use when animating. -1 for no constant animation.
	 */
	public CosmosEntity(GameCanvas game, String folder, String ref,
		...
		if (ref != null) {
			...
			for (int i = 0; i < imgs.length; i++) {
				if (sub != null) {
					sprites[i] = ImageLoader.get().getSprite(
						folder + "/" + imgs[i]);
				} else {
		...
		if (onDeath != null) {
			...
			for (int i = 0; i < imgs.length; i++) {
				if (sub != null) {
					onDeathSprites[i] = ImageLoader.get().getSprite(
						folder + "/death/" + imgs[i]);
				} else {
	 ...
	 */
	public CosmosEntity(GameCanvas game, String folder, String ref,
			String onDeath, int x, int y, long millis) {
		String[] imgs = null;
		String sub = folder;
		this.game = game;
		if (ref != null) {
	...
	public Point getPoint() {
		if (sprites != null && sprites[getCurrent()] != null) return new Point(
			(int) this.getX() + ((int) this.getWidth() / 2), (int) this.getY()
					+ ((int) this.getHeight() / 2));
		else
			return new Point((int) ((int) this.getX() + (me.width / 2)),
				(int) (this.getY() + (me.height / 2)));
	}
	...
	public void draw(Graphics g) {
		...
		if (CosmosConstants.debug) {
			...
			g.fillOval((int) (me.x + me.width / 2),
				(int) (me.y + me.height / 2), 3, 3);
		}
	}
