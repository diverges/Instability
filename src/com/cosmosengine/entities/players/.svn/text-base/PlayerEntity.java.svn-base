 ...
 * and the inventory.
 */
public class PlayerEntity extends CosmosEntity implements Killable, Clickable,
		KeyListener {
	 ...
	 * @param y
	 */
	public PlayerEntity(GameCanvas game, String folder, String ref,
	...
	public void draw(Graphics g) {
		...
		if (dealtDamage != -1) {
			...
			g.setColor(Color.RED);
			g.drawString("" + dealtDamage,
					(int) (game.player.getX() + (game.player.getWidth() / 2)),
					(int) (game.player.getY() - 5));
			if (drawStep >= 90) {
				dealtDamage = -1;
				drawStep = 0;
			}
	...
	public void keyPressed(KeyEvent e) {
		if (canMove) {
			...
			me.setBounds((int) me.x, (int) me.y, (int) me.width,
					(int) me.height);
		}
		...
		if (e.getKeyCode() == KeyEvent.VK_ENTER) {
			if (game.level != null) {
				if (game.level.isLoading) {
					...
					for (CosmosEntity obj : game.level
							.getlevelConstantObjects()) {
						if (obj.isAlive()) {
