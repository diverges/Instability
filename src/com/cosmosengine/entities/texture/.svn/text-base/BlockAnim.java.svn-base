...
public class BlockAnim extends BlockEntity {
	...
	public BlockAnim(GameCanvas game, String folder, String ref,
			String onDeath, String consAnim, int x, int y, int width,
			int height, int health, boolean respawn, Item onDeathDrop) {
		super(game, folder, ref, onDeath, x, y, width, height, onDeathDrop,
			health, respawn);
		String[] imgs = null;
		if (ref != null) {
			imgs = consAnim.split(";");
			...
			for (int i = 0; i < imgs.length; i++) {
				if (folder != null)
					constAnimSprites[i] = ImageLoader.get().getSprite(
						folder + "/constant/" + imgs[i]);
			}
		}
	}
	...
	public void draw(Graphics g) {
		if (isBeingCollidedDown || isBeingCollidedUp || isBeingCollidedLeft
				|| isBeingCollidedRight || dying) super.draw(g);
		else
