...
public class WallEntity extends CosmosEntity {
	...
	public WallEntity(GameCanvas game, String folder, String ref,
	...
	public void draw(Graphics g) {
		if (this.sprites != null) super.draw(g);
		if (CosmosConstants.debug) {
			g.setColor(Color.WHITE);
			if (this.rightColl != null && this.isBeingCollidedRight) {
				g.drawRect(rightColl.x, rightColl.y, rightColl.width,
					rightColl.height);
				g.drawString(rightColl.x + ", " + rightColl.y, rightColl.x,
					rightColl.y);
			}
			if (this.leftColl != null && this.isBeingCollidedLeft) {
				g.drawRect(leftColl.x, leftColl.y, leftColl.width,
					leftColl.height);
				g.drawString(leftColl.x + ", " + leftColl.y, leftColl.x,
					leftColl.y);
			}
			...
			if (this.downColl != null && this.isBeingCollidedDown) {
				g.drawRect(downColl.x, downColl.y, downColl.width,
					downColl.height);
				g.drawString(downColl.x + "," + downColl.y, downColl.x,
					downColl.y + (int) me.height);
			}
		}
	}
	...
	private boolean checkRight() {
		if (!isAlive || dying) return false;
		rightColl = new Rectangle((int) me.x - 4, (int) me.y,
	...
	private boolean checkLeft() {
		if (!isAlive || dying) return false;
		leftColl = new Rectangle((int) me.x, (int) me.y,
			(int) this.getWidth() + 4, (int) this.getHeight());
		return game.player.getBounds().intersects(leftColl);
	}

	private boolean checkDown() {
		if (!isAlive || dying) return false;
		downColl = new Rectangle((int) me.x, (int) me.y - 4,
			(int) this.getWidth(), (int) this.getHeight());
	...
	private boolean checkUp() {
		if (!isAlive || dying) return false;
		upColl = new Rectangle((int) me.x, (int) me.y, (int) this.getWidth(),
			(int) this.getHeight() + 4);
