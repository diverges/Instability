...
public class EventTriggerZoneEntity extends CosmosEntity {
	 ...
	 * @param y
	 * @param eventID
	 *            - specific call ID for level
	 * @param triggerOnce
	 *            - Specify the amount of times event should activate while in current level "0" false, "1" true
	 */
	public EventTriggerZoneEntity(GameCanvas game, int x, int y, int width,
			int height, String eventID, String triggerOnce) {
	...
	public void act() {
		if (isAlive) {
			...
			} else if (!wasEventTriggered && onlyTriggerOnce
					&& isEventTriggered) {
	...
	public void collision() {
		if (isAlive) {
			// redraw rectangle
			this.me = new Rectangle((int) me.x, (int) me.y, (int) me.width,
				(int) me.height);
		...
		if (!isAlive && !me.contains(game.player.getPoint())
				&& !onlyTriggerOnce) {
